...
class Dataset:
    ...
    def compute_measures(self, var, window=None):
        ...
        for w in window:
            ...
            res = stl.fit()
            strength_seasonality = 1- (np.var(res.resid))/(np.var(res.observed-res.trend))                                                       
            strength_trend = 1 - (np.var(res.resid))/(np.var(res.observed-res.seasonal))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--section', type=str, default='00', help='section of files')
    parser.add_argument('--mino', action='store_true', default=False, help='mino or P50')
    args = parser.parse_args()
    sec = args.section
    mino= args.mino

    data_path_mino = '/gpfs/projects/bsc28/bsc28642/Wind/Data/'
    data_path_P50 = 'e:/data_turbines/'
    path = 'e:/data_turbines/'
    path_out_mino = '/gpfs/projects/bsc28/bsc28498/Wind-local/Res/'
    path_out_P50 = 'e:/data_turbines/'

    path = '/gpfs/projects/bsc28/bsc28642/Wind/Data/'
    path1 = '/gpfs/projects/bsc28/bsc28498/'
    
    print (mino)
    if mino :
        data_path = data_path_mino
        path_out = path_out_mino 
    else:
        data_path = data_path_P50
        path_out = path_out_P50
    ...
    folder_files = list(glob.glob(data_path + sec+ '-*12.npy'))
    print(folder_files)
    ...
    for f in folder_files:
       ...
       mode = ('2D', '2D')
       dataset = Dataset(config={"datanames":[ f_name ], "vars": [0]}, data_path=data_path)
       wind_res_path = 'e:/data_turbines/test'
       dataset.load_raw_data()
#    config.update({'_id':str(int(time.time()))})

       a = (dataset.compute_measures(var=0,window={'12h':12, '24h':24, '1w':168, '1m':720, '3m':2190, '6m':4380}))
       config["result"]["wind_speed"]=a
       config["_id"]   = str(int(time.time()))
       config["etime"] = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())
       with open(path_out+str(config["_id"])+'.json', 'w') as outfile:
