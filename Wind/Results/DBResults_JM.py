...
: añadida funcion de extraer de la columna de un dataframe. OJO transfiere las columnas marcadas como #JM a futuras versiones
...
class DBResults:
    ...
    def __init__(self, conn=mongoconnection, test="", error='R2'):
        ...
        self.error = error
# JM        
    def retrieve_results_dataframe(self, df):
        # df dataframe = indx, test, validation
        ldata = []
        ldata = df.values.tolist()
        ldata_a = np.array(ldata)
        a = ldata_a[:,0].astype(int)  # ni puta idea porque lo deja como float
        ldata = sorted(ldata, key=lambda x: x[0])
        self.exp_result['sites'] = a
