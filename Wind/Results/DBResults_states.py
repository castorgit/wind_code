...
class DBResults:
    ...
    def __init__(self, conn=mongoconnection, test="", error='R2'):
        ...
        if os.path.isfile(f'{wind_data_path}/states.csv'):
            self.df_states = pd.read_csv(wind_data_path + '/states.csv')
#       else:
#            warnings.warn('No states file found, maps with states will not be available')
        self.exp_result={}
        self.exp_result2={}
        self.exp_lresults=[]
        self.error = error
...
#            self.exp_df.drop(columns=['hour', 'site'], inplace=True)
    ...
    def select_geo_state(self, state):
        """
        ...
        """
        def calc_state(num):
           n = list(self.df_states.loc[self.df_states['numero_site'] == int(num)]['state'])
           # print(num,'->', n[0])
           return (n[0])
       
        #print (calc_state(self.coords[self.exp_result['sites'][100]][2]))
        #print (calc_state(self.coords[self.exp_result['sites'][75538]][2]))
        
        #print (calc_state(self.coords[self.exp_result['sites'][75538]][2])==state)
        self.selection = [i for i in self.osel if
                          (state == calc_state(self.coords[self.exp_result['sites'][i]][2]))]
    ...
    def plot_hours_boxplot_compare(self, dset=('val', 'test'), figsize=(16, 4), title=None, labels=None, font=None, save=None, saveformat='pdf'):
        ...
        g = sns.boxplot(x='hour', y='acc', hue=title,data=df)


        handles, _ = g.get_legend_handles_labels()
#       g.legend(handles, [ "test - Random Forest ", "test - CNN separable 2 Layers"])
