...
if __name__ == '__main__':
    ...
    sec = '109-'
    print(path + sec+ '*12.npy')
    
    start = time.time()
    l = []

    folder_files = list(glob.glob(path + sec+ '*12.npy'))
    ...
    for f in folder_files:
       ...
       mode = ('2D', '2D')
       dataset = Dataset(config={"datanames": ["11-5709-12"], "vars": [0]}, data_path='e:/data_turbines/')
...
#    config.update({'_id':str(int(time.time()))})
       ...
       with open(path+str(config["_id"])+'.json', 'w') as outfile:
