...
class CNNS2SCrazyIvanArchitecture(NNS2SArchitecture):
    ...
    def generate_model(self):
        ...
        convomodel = Conv1D(filters[0], input_shape=(idimensions), kernel_size=kernel_size[0], strides=strides[0],
                              padding='causal', kernel_regularizer=k_regularizer)(input)
        convooutput = generate_activation(activation)(convomodel)
        ...
        for k in kernel_size:
            convomodel = Conv1D(filters[0], input_shape=(idimensions), kernel_size=k, strides=strides[0],
                              padding='same', kernel_regularizer=k_regularizer)(convooutput)
