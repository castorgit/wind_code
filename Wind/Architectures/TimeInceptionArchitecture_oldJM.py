...
from keras.layers import Dense, Dropout, Conv1D, Flatten, Concatenate, Input, MaxPool1D,GlobalAveragePooling1D
...
#        strides = self.config['arch']['strides']
        ...
        for k in inc_filters:                   # len(inc_filters)=number 
           ...
           if bottleneck_filters[i] !=0:      #Create Bottleneck?
              ...
              r = r + 1
              bottleneck_tensor = Conv1D(filters=bottleneck_filters[i], kernel_size=1,
           ...
           for j in range(0,3):
              ...
              inc_layer = Conv1D(k, kernel_size=inc_kernels[i][j], strides=1,
           ...
           max_pool1 = Conv1D(filters=32, kernel_size=1,
