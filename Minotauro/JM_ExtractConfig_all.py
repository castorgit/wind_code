...
if __name__ == '__main__':
    ...
    bsc_path = 'e:/Wind/Minotauro/Jobs/'
    ...
    for nr in range(args.nrep):
        ...
        if not args.bsc:
            ...
            nm = f"{strftime('%Y%m%d%H%M%S')}{str(nr).zfill(2)}"
...
#SBATCH --job-name="{nm}{str(nr).zfill(3)}"
#SBATCH -D{jobs_code_path}/Experiments
#SBATCH --output={jobs_root_path}/Run/windjobmino{nm}{str(nr).zfill(3)}.out
#SBATCH --error={jobs_root_path}/Run/windjobmino{nm}{str(nr).zfill(3)}.err
#SBATCH --ntasks=1
...
"""
        ...
        if args.machine == 'mino':
            batchjob = open(f"{spath}/windjobmino{nm}{str(nr).zfill(3)}.cmd", 'w') # jm
        ...
        if len(lconfig) > 0:
            for config in tqdm(lconfig):
                ...
                fconf = open(f"{spath}/{config['_id']}.json", 'w')         # jm
                fconf.write(sconf + '\n')
                fconf.close()
                if args.copy:
